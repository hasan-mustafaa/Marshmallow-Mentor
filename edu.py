...
def pick_target_point_from_tokens(
    ...
    if feedback.error_type == "parse_error":
        first = sorted(tokens, key=lambda t: (_center(t["bbox"])[1], _center(t["bbox"])[0]))[0]
...
def make_grid(image_width: int, image_height: int, rows: int, cols: int) -> List[List[Dict[str, float]]]:
    """
...
async def analyze_image(file: UploadFile = File(...)):
    ...
    if not equation_str:
        raise HTTPException(status_code=422, detail="Could not extract equation from image")
